...
int menorCarta(wchar_t *cartas) 
{
    ...
    for (int i = 1; i < numCartas; i++) // itera o conjunto de cartas
    {
        if (cartas[i] % 16 < menorCarta % 16)
            // se a carta for maior
            menorCarta = cartas[i];
        else if (cartas[i] % 16 == menorCarta % 16 && cartas[i] < menorCarta)
            // se carta for igual e naipe maior
            menorCarta = cartas[i];
    }
    return menorCarta % 16;
}
...
int conjunto(wchar_t cartas[], int numCartas) // verifica se é um conjunto
{
    ...
    {
        if (cartas[0] % 16 != cartas[i] % 16)
            return 0; // se as cartas forem diferentes
        for (int j = 0; j < numCartas; j++)
        {
            if (cartas[i] == cartas[j] && i != j)
                return 0; // se houver cartas iguais
        }
    }
    return 1;
}
...
int seq(wchar_t cartas[], int numCartas, int mult2) // verifica se é uma sequência
{
    ...
    if (numCartas <= 2)
        return 0; // se não tem cartas suficientes para formar uma sequência
    if (mult2 == 2 && numCartas % 2 != 0)
        return 0; // se for uma dupla sequência e o número de cartas não for par

    if (mult2 == 2)
        numCartas /= 2; // se for uma dupla sequência
    ...
    for (int i = 0; i < numCartas; i++) // itera o conjunto de cartas por ordem crescente
    {
        ...
        if (i == numCartas - 1)
            temConsecutivo = 1; // a última carta tem consecutivo por defeito
        else
            temConsecutivo = checkConsecutivo(cartas, numCartas, mult2, cartaAtual, &indexConsecutivo); // teste se tem consecutivo

        if (mult2 == 2)
            temImagem = checkImagem(cartas, numCartas, mult2, cartaAtual); // testa se tem imagem (caso dupla sequencia)

        if (temConsecutivo && temImagem)
            cartaAtual = cartas[indexConsecutivo];
        if (temConsecutivo == 0 || temImagem == 0)
            return 0; // se não tiver uma carta consecutiva ou imagem
    }
...
int combinacoesIguais(wchar_t cartas[][100], int linhas)
{
    ...
    {
        if (conjunto(cartas[n], wcslen(cartas[n])))
            eConjunto = 1;
        else
        {
            eConjunto = 0;
    ...
    for (int n = 0; n < linhas; n++)
    {
        if (seq(cartas[n], wcslen(cartas[n]), 1))
            eSequencia = 1;
        else
        {
            eSequencia = 0;
    ...
    for (int n = 0; n < linhas; n++)
    {
        if (seq(cartas[n], wcslen(cartas[n]) / 2, 2))
            eDuplaSequencia = 1;
        else
        {
            eDuplaSequencia = 0;
    ...
    if (eConjunto || eSequencia || eDuplaSequencia)
        return 1;
    else
        return 0;
}
